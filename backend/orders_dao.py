...
def get_order_details(connection, order_id):
    ...
    query = "SELECT order_details.order_id, order_details.quantity, order_details.total_price, "\
            "product.product_name, product.product_Price FROM order_details LEFT JOIN product on " \
    ...
    records = []
    for (order_id, quantity, total_price, product_name, product_Price) in cursor:
        records.append({
            'order_id': order_id,
            'quantity': quantity,
